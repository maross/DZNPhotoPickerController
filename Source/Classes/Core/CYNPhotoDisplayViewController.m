//
//  CYNPhotoDisplayViewController.m
//  Morphcast
//
...
@interface CYNPhotoDisplayViewController () <UICollectionViewDelegateFlowLayout, UITableViewDelegate,
...
@implementation CYNPhotoDisplayViewController
...
- (instancetype)initWithCollectionViewLayout:(UICollectionViewLayout *)layout
{
    ...
    self = [super initWithCollectionViewLayout:[CYNPhotoDisplayViewController layoutFittingSize:size]];
    if (self) {
        [self commontInit];
    }
    return self;
...
- (void)loadView
{
    ...
    _segmentedControlTitles = NSArrayFromServices(self.controller.supportedServices);
    NSAssert((_segmentedControlTitles.count <= 4), @"DZNPhotoPickerController doesn't support more than 4 photo service providers");
    
    _selectedService = DZNFirstPhotoServiceFromPhotoServices(self.controller.supportedServices);
...
- (BOOL)canSearchTags
{
    if (!self.controller.allowAutoCompletedSearch) {
        return NO;
    }
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)elementKind atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([elementKind isEqualToString:UICollectionElementKindSectionHeader]) {
        ...
        if ([self canDisplayFooterView]) {
            if (self.isLoading || self.controller.infiniteScrollingEnabled) {
...
- (void)collectionView:(UICollectionView *)collectionView willDisplaySupplementaryView:(UICollectionReusableView *)view forElementKind:(NSString *)elementKind atIndexPath:(NSIndexPath *)indexPath
{
    if ([elementKind isEqualToString:UICollectionElementKindSectionFooter]) {
        if (self.controller.infiniteScrollingEnabled && [self canDisplayFooterView] && !self.isLoading) {
